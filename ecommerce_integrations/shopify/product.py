...
# ---------------- ONE-WAY CREATION ENFORCEMENT ----------------
def create_items_if_not_exist(order):
    """Disabled to enforce one-way item creation (ERPNext ➜ Shopify only)."""
    return
# --------------------------------------------------------------
...
def upload_erpnext_item(doc, method=None):
    ...
    # Only push if explicitly selected
    if not cint(getattr(item, "custom_sync_to_shopify", 0)):
        return
    ...
    if is_new_product:
        ...
        # ensure image on create
        _apply_erp_image_to_shopify_product(product, template_item)
        ...
        try:
            ...
            # Mapping exists but Shopify product is gone. Clean mapping & create a fresh product.
            ...
            # Recreate mapping rows for template and item as needed
            ecom_items = list(set([item, template_item]))
            for d in ecom_items:
                ecom_item = frappe.get_doc(
                    {
            ...
            write_upload_log(status=True, product=product, item=item, action="Recreated")


def _apply_erp_image_to_shopify_product(shopify_product: Product, erpnext_item) -> None:
    """Ensure Shopify product has the same primary image as ERPNext Item.image."""
    try:
        img_src = _get_erp_item_image_url(erpnext_item)
        if not img_src:
            return
        if getattr(shopify_product, "images", None):
            try:
                shopify_product.images[0]["src"] = img_src
            except Exception:
                try:
                    shopify_product.images[0].src = img_src
                except Exception:
                    shopify_product.images = [{"src": img_src}]
        else:
            shopify_product.images = [{"src": img_src}]
    except Exception:
        # Never block the sync on image issues.
        pass


def _get_erp_item_image_url(erpnext_item) -> Optional[str]:
    """Return absolute URL for ERPNext Item.image if present."""
    path = getattr(erpnext_item, "image", None)
    if not path:
        return None
    try:
        return get_url(path)
